...
const getGenreFilms = (filmsArr: Film[]) => {
    return filmsArr.reduce((acc: string[], film: Film) => {
        acc.push(...film.genre)
        return acc;
    },[])
}
const getUniqGenre = (getGenreFilms: any) => {
    const genreArr = getGenreFilms(filmsArr)
    return genreArr.reduce((acc: string[], val: string) => {
...
// повторения)
const getActorsFilms = (filmsArr: Film[]) => {
    return filmsArr.reduce((acc: string[], film: Film) => {
        acc.push(...film.actors)
        return acc;
    },[])
}
const getUniqActor = (getActorsFilms: any) => {
